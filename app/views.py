		...
		try:
			...
			if serializer.is_valid(raise_exception=True):
				if f_objs.count():
					...
					f_obj.delete()
					serializer.save(Invoice_id="Invoice_"+str(int(iid)+1),Bill_no=int(bn)+1,Ref_no="Ref_"+str(uuid.uuid4())[:10],Total=int(data["Quantity"])*int(data["Plan_Cost"]),Grand_total=(int(data["Quantity"])*int(data["Plan_Cost"]))+320+int(data["Installation"]))
				else:	
					serializer.save(Ref_no="Ref_"+str(uuid.uuid4())[:10],Total=int(data["Quantity"])*int(data["Plan_Cost"]),Grand_total=(int(data["Quantity"])*int(data["Plan_Cost"]))+320+int(data["Installation"]))
